# Create a demonstration script that shows how to use the system
demo_code = '''
#!/usr/bin/env python3
"""
...
# Also create a simpler usage example
usage_example = '''
...
print("Testing the RandomizerEngine with example generators...")

# Import our engine
exec(open('RandomizerEngine.py').read())

# Create engine and load generators
engine = RandomizerEngine()

# Load the generators we created
with open('televangelist_generator.json', 'r') as f:
    tv_data = json.load(f)
tv_name = engine.load_generator(tv_data)

with open('satanic_panic_generator.json', 'r') as f:
    sp_data = json.load(f)
sp_name = engine.load_generator(sp_data)

print(f"\n🎭 TELEVANGELIST GENERATOR EXAMPLES:")
for i in range(3):
    result = engine.generate(tv_name)
    print(f"{i+1}. {result}")

print(f"\n📺 SATANIC PANIC GENERATOR EXAMPLES:")  
for i in range(3):
    result = engine.generate(sp_name)
    print(f"{i+1}. {result}")

print(f"\nFiles created:")
print("- demo.py (interactive demonstration)")
print("- usage_example.py (simple usage guide)")
print("- System successfully tested with example generators!")